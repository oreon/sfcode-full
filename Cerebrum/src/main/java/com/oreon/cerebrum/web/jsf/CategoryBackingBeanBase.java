...
public class CategoryBackingBeanBase extends BaseBackingBean<Category> {
	...
	private List<DrugCode> listDrugCodes = new ArrayList<DrugCode>();
	...
	public List<DrugCode> getListDrugCodes() {
		if (listDrugCodes.isEmpty())
			loadDrugCodes();

		return listDrugCodes;
	}

	public void setListDrugCodes(List<DrugCode> listDrugCodes) {
		this.listDrugCodes = listDrugCodes;
	}

	private void loadDrugCodes() {
		listDrugCodes.clear();
		if (category != null) {
			listDrugCodes.addAll(category.getDrugCode());
		}
		int sizeOfExistingElements = listDrugCodes.size();
		// add a few spare rows - lets say parent has 3 children and we need to
		// show 5 rows - then add 2 rows with 2 new parents
		for (int i = 0; i < INITIAL_RECORDS - sizeOfExistingElements; i++) {
			listDrugCodes.add(new DrugCode());
		}
	}

	private void addDrugCodesToCategory() {
		category = categoryService.save(category);//To prevent lazy initialization exception
		category.getDrugCode().clear();
		List<DrugCode> listValidDrugCodes = new ArrayList<DrugCode>();

		for (DrugCode drugCode : listDrugCodes) {

			drugCode.setCategory(category);
			listValidDrugCodes.add(drugCode);

		}

		category.getDrugCode().addAll(listValidDrugCodes);
		resetCachedLists();
	}
	 ...
	 * @param actionEvent
	 */
	public void addNewDrugCodeRow(ActionEvent actionEvent) {
		listDrugCodes.add(new DrugCode());
	}
	 ...
	 * @param actionEvent
	 */
	public void deleteDrugCodeRow(ActionEvent actionEvent) {
		String rowIndex = (String) FacesContext.getCurrentInstance()
				.getExternalContext().getRequestParameterMap().get(
						"deleteRowIndex");

		int index = Integer.parseInt(rowIndex);
		DrugCode drugCode = listDrugCodes.get(index);
		listDrugCodes.remove(index);
