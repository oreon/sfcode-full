...
/*extern*/ bool    UseClose=false; // закрытие п112тку PipStep. рекомендутс€ false 
...
extern double  LotExponent=1.0; // умножение лотов в серии по експоненте дл€ вывода в безубыток. первый лот 0.1, сери€: 0.16, 0.26, 0.43 ...

extern double  slip=5;   // допустимое проскальзывание цены в пипсах
extern double  Lots=0.02; // теперь можно и микролоты 0.01 при этом если стоит 0.1 то следующий лот в серии будет 0.16
extern double  TakeProfit=10; // ”ровень прибыли в пипсаз от цены открыти€.
double  Stoploss=3000; // эти три параметра не работают
double  TrailStart=10;
double  TrailStop=10;
extern double  PipStep=50; // расто€нию в пипсах убытка на котором открываетьс€ следующий ордер колена.
extern int     MaxTrades=10;
extern bool    UseEquityStop=true;
extern double  TotalEquityRisk=80; //loss as a percentage of equity
...
//+------------------------------------------------------------------+
  int start()
  {
      int score  = calculateMacd();
  
     if(IsDemo()){
       Comment("XPips-V2-Demo By Neural Traders corp. \n\n ATR: " + iATR(NULL,0,12,0) +    getTrendMessage(score));
     }else{
      if(!validateKey(Key))
          return (false);
      else{
         
          Comment("XPips-V2 By Neural Traders corp. \n\n ATR: " + iATR(NULL,0,12,0) + "" +  getTrendMessage(score));
      }
     }
 ...
 */
 ...
 int calculateMacd(){
   ...
   score += macdScore(PERIOD_H1);
   score += macdScore(PERIOD_M30);
   
  
   return (score);
 }
 
 string getTrendMessage(int score){
   if(score >= 2 ){
