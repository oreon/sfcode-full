...
public class ColumnTag extends AbstractOutputHtmlTag {
	...
	public int doEndTag() throws JspException {
		try {
			...
			if (parent.getCurrentDisplayRow() == CurrentDisplayRow.MODE_HEADER) {// set
																					// headers
				createHeader();
			} else if (parent.getCurrentDisplayRow() == CurrentDisplayRow.MODE_FILTER) {
				createFilterCell();
			} else
	...
	private void createHeader() throws IOException {
		parent.getWriter().print(
				"<th><a href='?sortBy='" + getName() + "'>"
						+ getLabelToRender() + "</a></th>");
	}

	private void createCellContent(Object bean) throws IOException {
		if (bean != null) {
			String property = String.valueOf(ReflectionBasedPopulator
					.safeGetValue(bean, getName()));
			parent.getWriter().print(
					"<td>" + getLinkToRenderBegin()
							+ ViewHelper.getEmptyIfNull(property)
							+ getLinkToRenderEnd() + "</td>");
		}
	}

	private void createFilterCell() throws IOException {
		parent.getWriter().print(
				"<td><input type=\"text\" name=\"" + getName()
						+ "\" size='4' /></td>");
	}

	String getLabelToRender() {
		return label == null ? ViewHelper
				.convertCamelCaseToTitleCase(getName()) : getLabel();
	}
	...
	String getIdToRender() {
		Object bean = pageContext.getAttribute("currentElement");
		return param != null ? (param + "=" + ReflectionBasedPopulator
				.safeGetValue(bean, param)) : StringUtils.EMPTY;
	}
