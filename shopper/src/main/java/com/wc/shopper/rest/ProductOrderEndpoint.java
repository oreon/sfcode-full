...
public class ProductOrderEndpoint
{
   ...
   public ProductOrder deleteById(@PathParam("id") Long id)
   {
      em.joinTransaction();
   ...
   @Path("/{id:[0-9][0-9]*}")
   @Produces("application/xml")
   public ProductOrder findById(@PathParam("id") Long id)
   {
   ...
   @Path("/{id:[0-9][0-9]*}")
   @Consumes("application/xml")
   public ProductOrder update(@PathParam("id") Long id, ProductOrder entity)
   {
      entity.setId(id);
      em.joinTransaction();
