...
public abstract class BaseAction<T extends BusinessEntity> extends
	...
	private static final int MYSQL_UNIQUE_EXCEPTION_CODE = 1062;
		...
		try {
			...
			if (pe.getCause() instanceof ConstraintViolationException) {
				...
				if (pe.getCause().getCause() instanceof SQLException) {
					...
					if (se.getErrorCode() == MYSQL_UNIQUE_EXCEPTION_CODE) {

						String[] tokens = se.getMessage().split(" ");
						throw new UniqueException("A Record with "
								+ tokens[tokens.length - 1] + " "  + tokens[tokens.length - 4]
								+ " already exists !" + se.getMessage());
					}
				} else {
					throw new BusinessException ("A Constraint violation Error Occured: "
							+ pe.getMessage());
				}
			...
			addErrorMessage("Error Saving record: " + ex.getMessage());
			...
			if (pe.getCause() instanceof ConstraintViolationException) {
				...
				if (pe.getCause().getCause() instanceof SQLException) {
					...
					if (se.getErrorCode() == 1062) {
						...
						String[] tokens = se.getMessage().split(" ");
						throw new BusinessException("A Record with this "
								+ tokens[tokens.length - 1]
								+ " already exists !");
					}
				} else {
					addErrorMessage("A Constraint violation Error Occured: "
							+ pe.getMessage());
				}
				return "error";
			}
	...
	protected String saveWithEx() {
		try {
			return doSave();
		} catch (BusinessException be) {
			addErrorMessage(be.getMessage());
		} catch (Exception e) {
			addErrorMessage("AN ERROR OCCURED " + e.getMessage());
			log.error("error saving record ", e.getMessage());
		}
		return "failure";
	}

	protected String saveWithEx(T t) {
		try {
			setInstance(t);
