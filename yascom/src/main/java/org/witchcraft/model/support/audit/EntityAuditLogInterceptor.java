...
public class EntityAuditLogInterceptor extends BaseAction<AuditLog>{
	...
	private void saveAuditLog(EventTypes action, Object entity) {
		...
		if (entity instanceof Auditable) {
			...
			AuditLog auditLog = new AuditLog(action, BaseEntity,  entity.getClass().getCanonicalName(), BaseEntity.getId(),  userName);
			persist(auditLog);
		}
	}
	...
	public List<AuditLog> getAuditLogsForEntityAndId(String name, Long id){
		...
		Query query = entityManager.createQuery(qryString).setParameter(1, name).setParameter(2, id);
		return query.getResultList();
	}
