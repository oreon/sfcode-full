...
public abstract class StudentVitalInfoActionBase
		extends
			BaseAction<StudentVitalInfo> implements java.io.Serializable {

	@In(create = true)
	@Out(required = false)
	@DataModelSelection
	private StudentVitalInfo studentVitalInfo;

	@In(create = true, value = "studentAction")
	com.oreon.smartsis.web.action.domain.StudentAction studentAction;

	@DataModel
	private List<StudentVitalInfo> studentVitalInfoRecordList;

	public void setStudentVitalInfoId(Long id) {
		if (id == 0) {
			clearInstance();
			clearLists();
			loadAssociations();
	 ...
	 * @param id
	 */
	public void setStudentVitalInfoIdForModalDlg(Long id) {
		setId(id);
		clearLists();
		loadAssociations();
	}
	...
	public Long getStudentVitalInfoId() {
		return (Long) getId();
	}

	public StudentVitalInfo getEntity() {
		return studentVitalInfo;
	}

	//@Override
	public void setEntity(StudentVitalInfo t) {
		this.studentVitalInfo = t;
		loadAssociations();
	}

	public StudentVitalInfo getStudentVitalInfo() {
		return (StudentVitalInfo) getInstance();
	}
	...
	public StudentVitalInfo getDefinedInstance() {
		return (StudentVitalInfo) (isIdDefined() ? getInstance() : null);
	}
	...
	public void addAssociations(Criteria criteria) {
		...
		if (studentVitalInfo.getStudent() != null) {
			criteria = criteria.add(Restrictions.eq("student.id",
					studentVitalInfo.getStudent().getId()));
		}
