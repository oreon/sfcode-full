...
public class ExamBackingBean extends ExamBackingBeanBase {
	...
	public void selectExam() {
		...
		String idStr = (String) context.getExternalContext()
	...
	public String launchExam() {
		...
		if (candidate == null) {
			...
			createErrorMessage("You need to login or register to continue");
			return NavigationConstants.LOGIN_OR_REGISTER;
		}
		exam = examService.load(exam.getId());
		examFactory = ServiceFacade.getInstance()
				.getRandomQuestionsExamFactoryImpl();
		...
		log.info("Launching exam " + instance.getExam().getName() + " "
				+ instance.getAnsweredQuestionCount() + "questions ");
